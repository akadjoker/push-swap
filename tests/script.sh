...
testing() {
	...
	if [ "$result_push_swap" = "Error" ] || [ "$result_push_swap" = "" ]; then
		...
		printf "\e[33mvalue_instr\e[0m: $value_instr ✅";
	fi
	printf "\n"
...
check_parameters() {
	...
	if ! [[ $1 =~ $re ]]; then
		...
		echo "value_test must be positive number" >&2; exit 1;
	fi
}

check_value_parameters() {
	if [ "$1" -ne 1 ] && [ "$1" -ne 2 ]; then
		printf "$0 size_array [value_test]\n";
		exit 1;
	fi
}
...
main() {
	...
	check_parameters $1 $value;

	declare -a array
	fill_array $1;

	for ((n=0; n<value; n++)); do
		shuffle $1
		create_string $1
